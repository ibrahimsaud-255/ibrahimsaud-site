...
/* ═══════════════════════════════════════════════════════════════════════
   بحثٌ عن معلّم واشتراكاته وطلبات Telr — للتشخيص والتفعيل اليدويّ
   ═══════════════════════════════════════════════════════════════════════ */
...
export interface TelrOrderRow {
  cartRef: string;
  userId: string;
  planId: string;
  status: "pending" | "paid" | "failed" | "cancelled";
  telrTranRef: string | null;
  createdAt: string;
  updatedAt: string;
}
...
export interface SubscriptionRow {
  id: string;
  userId: string;
  platform: "revenuecat" | "telr" | "manual" | "organization";
...
/** يجد المعلّم بالبريد ويعيد طلباته واشتراكاته — للتشخيص. */
export function lookupTeacher(email: string) {
  const q = encodeURIComponent(email.trim());
  return api<TeacherLookup>(`admin/telr/orders-by-email?email=${q}`);
}

/** يعيد تحقّق طلبٍ محدّد من Telr Check API ويفعّل الاشتراك إن كان مدفوعاً. */
export function verifyTelrOrder(cartRef: string) {
  return api<{ status: string; cartRef: string; userId: string; planId: string }>(
    "admin/telr/verify-payment",
    { method: "POST", body: { cartRef } },
  );
}

/** يحذف اشتراكاً يدويّاً بمعرّفه — استُعمل لإلغاء منحةٍ منحت خطأً. */
export function deleteGrant(id: string) {
  return api<{ success: boolean }>(`admin/grants/${encodeURIComponent(id)}`, {
    method: "DELETE",
  });
}
